...
public class ReplayEventsAtOffset
  ...
  public static class Output {
    ...
    public Output(Long startOffset) {
  ...
  @Override
  public Response<Input> apply(ConfigResource resource, Input input) throws RestApiException {
    if (input == null) {
    ...
    if (input.offset < 0) {
      Map<String, Long> currentOffsets = brokerApiSubscribers.getOffsetsForTopic(input.topic);
      ...
      long currentOffset = currentOffsets.values().stream().mapToLong(Long::longValue).max().orElse(0);
    ...
    logger.atFine().log(
        "Successfully replayed events for topic %s at offset %d", input.topic, replayOffset);
