...
public abstract class StreamSession implements Session {
  ...
  private boolean streamConfigIsValid() {
    List<String> missingSettings = new ArrayList<>(3);

    Stream stream = properties.getSection(Stream.class);
    if (stream.uri.isEmpty()) missingSettings.add("uri");
    if (stream.streamPrefix.isEmpty()) missingSettings.add("streamPrefix");
    if (stream.consumerPrefix.isEmpty()) missingSettings.add("consumerPrefix");
