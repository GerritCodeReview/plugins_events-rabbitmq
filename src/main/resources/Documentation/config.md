...
Known Limitation: If the RabbitMQ cluster is not configured to use node names with the full FQDN,
each cluster node must be on the same domain as configured by stream.uri. Contributions welcome.
