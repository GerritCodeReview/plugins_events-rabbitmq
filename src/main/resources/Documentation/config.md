...
You can locate config files to `$site_path/data/events-rabbitmq/site`.
File extension must be `.config`.
Connection to RabbitMQ will be established for each files.

If `events-rabbitmq.config` exists in `$site_path/data/events-rabbitmq`, it is loaded at first.
